...
import { BASE_EXERCISES, BANDS, safeKey, CHIN_KEY, APP_VERSION, IS_DEV } from "./config.js";
import {
...
function renderWeightsList() {
  ...
  const names = allExNames().slice().sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }));
...
function renderSexGroups(board, rowFn, hasRecord = (e) => e.value !== null, restSortFn = (a, b) => (b.value ?? -1) - (a.value ?? -1)) {
...
function renderChallengeDetail() {
  ...
  const chRestSort = isTime
    ? (a, b) => (a.entry?.value ?? Infinity) - (b.entry?.value ?? Infinity)
    : (a, b) => (b.entry?.value ?? -1) - (a.entry?.value ?? -1);
  const rows = clientsSorted.length === 0
    ? `<div class="empty">${t.noClientsChallenge}</div>`
    : renderSexGroups(clientsSorted, chRowFn, (e) => e.entry !== null, chRestSort);
...
function appFooter() {
  ...
  return `
    <div class="app-footer">
      ...
      ${t.footerName}${(IS_DEV || APP_VERSION.includes('BUILD_')) ? '' : ` <span class="app-footer-version">${APP_VERSION}</span>`}
